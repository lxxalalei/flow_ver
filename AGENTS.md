...
- Skill 的任务模型只使用 `user_role`、`resource_target`、目标和显式约束。未知字段保持
  未知，不为补齐模型而追问；但该边界只适用于不改变搜索方向的边缘信息。凡是不同答案会显著改变
  搜索方向、内容尺度或筛选标准的维度，都不属于“保持未知”范畴，宽主题必须先澄清收敛
  再搜索。搜索方向必须由资源对象、目标和显式约束共同决定，不能仅从当前对话者身份推出
  额外内容方向。
...
- 只允许 `http`/`https`；服务端强制执行域名策略、超时、重试上限、并发限制、内容类型和真实文件格式校验；文件下载以固定数据块流式写入，不设置每资源字节上限。
- 归档只接受 `asset_id`，不得接受模型提供的任意文件路径。
- 大文件不得写入模型上下文或 Tool JSON；返回资产 ID、元数据或受控访问地址。

以上是本项目已有明确架构要求，不构成 Agent 继续自行增加新安全框架的授权。
...
- 下载逻辑修改必须覆盖与 diff 直接相关的流式写入、取消、幂等或非法状态转换测试；不因其中某一项存在就自动要求全仓测试。
