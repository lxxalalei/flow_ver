...
本仓库同时是专用 OpenClaw Agent 的源码工作区和本地开发工作区，不是教育平台的生产多租户运行目录。正式凭据、下载资产、平台会话和用户运行数据必须与源码分离；`.openclaw-test/` 仅用于临时隔离测试。

## 项目目标

最终产品目标不是复刻旧七个 Skill 或旧六阶段文件流水线，而是让孩子或家长直接通过自然语言完成可信的教育资源闭环：表达需求、获得必要且克制的澄清、探索合适来源、理解候选差异、明确选择、获取真实资源，并在需要时归档。

当前架构以 `docs/CURRENT_ARCHITECTURE.md` 为准。旧 Flow / ResultSet / Presentation / Selection / Plan / authority / digest 等设计只保留在 Git 历史或 legacy 中，不是 active 架构兼容目标。

当前 active 产品部分：

- `skills/learning-resource-flow/`：唯一用户入口和语义决策层；
- `mcp/education-resources/`：资源搜索、URL 导入、Inspect、下载、Batch、Job 和归档能力；
- `mcp/session-manager/`：平台登录态引导、最小凭据提取与本地会话保存。
...
以下改动默认不允许直接引入：新的 canonical model、projection、duplicated state、同步层、对简单 API 的额外 adapter/wrapper、trivial persistence 的 repository 抽象、event bus、CQRS、自定义工作流状态机、compatibility layer、generalized framework、speculative extension point。
...
- 不根据历史交接猜测 `.gitignore` 或当前工作区状态；修改前以 live 仓库事实为准。
...
Skill 不负责复制 MCP 的参数表、返回字段、Provider 内部实现、Job 文件结构或平台 Registry。运行时 Tool schema 和真实返回是能力契约的事实来源。

不得把正常用户行为重新建模成 Flow、Presentation、Selection、Plan、eligibility、authority、binding、digest 等持久状态。

## education-resources MCP

`education-resources` 是资源能力和 IO 数据面，不是资源工作流后端。

当前核心能力是：平台搜索、创作者预览、外部 URL 导入、Inspect、下载、Job 状态/取消、完整 Batch 枚举与分页读取、文件归档。

状态边界：

- `resource_id` 是当前 MCP 进程内的临时操作句柄，不持久化；
- 真正稳定的资源身份是 URL、平台原生稳定 ID 等；
- `job_id` 只为真实长任务提供持久运行身份，用于下载/批量任务的进度、取消和重启后的状态；
- 不持久化 Flow、Selection、Plan、Asset、Outcome 或证明链。

用户已经明确选择资源并明确要求下载时，可直接调用下载能力，不增加 `prepare -> confirm -> start` 形式化二次确认。成功与否只依据真实 Job/文件结果。

Batch 的分页大小只控制单次 Tool Result，不得变成完整枚举的数据上限。除非用户明确要求“最多 N 条”，否则完整枚举不得偷偷加 `max_items`。

归档由 Agent 决定分类，MCP 只移动下载 Job 已产生的真实文件；不要恢复 `asset_id`、ArchiveRecord、AssetBundle 或 archive digest/version 状态链。

## session-manager MCP

`session-manager` 只负责平台登录态能力：返回登录引导、接收宿主浏览器捕获、按平台规则提取最小会话数据、保存/删除并按需验证状态。

- 用户自己完成登录；Agent 不索取或代填密码、验证码、短信码或 MFA。
- 浏览器捕获的数据由 session-manager 服务端筛选，Agent 不手工拼接 canonical Cookie/Token。
- 只有用户明确授权并主动提供合法取得的 canonical Cookie/Token 时，才按 Tool schema 直接保存。
- session-manager 的安全存储和幂等规则只约束凭据写入，不得扩散成 education-resources 的通用业务工作流状态。
...
不新增与真实业务无关的证明链、哈希绑定、任意大小限制、输入输出截断或“安全起见”的数据丢弃。

大结果使用 Job/文件/分页承载，不把完整大数据塞进模型上下文。

## Tool 与契约要求

- Tool 名称、description 和 input schema 应足以让模型理解“这个能力做什么、什么时候适合使用、参数怎么传”。
...
- 只有 release、跨切面高风险改动或明确要求时才默认考虑全量回归。
