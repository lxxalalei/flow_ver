...
description: 为孩子及其家长规划主题开放的学习资料搜索。将已确认的需求转化为多平台搜索计划，理解学习主题、目标、适龄表达、使用场景和资源形态，动态判断去哪里搜索、各平台搜索什么；生成查询词和真实接口参数，但不执行搜索、筛选结果或下载资源。
---
...
为孩子及其家长搜集能够帮助理解、探索、练习或实践的学习资料。先判断资源由孩子直接使用，还是由家长理解、选择或组织后陪伴孩子使用；搜索方向和平台选择应与这个角色一致。
...
**互补形态检查（必须执行）**：读取 Intent 的 `resource_types`、`format_preferences`、学习目标和使用场景。没有限定资源形态时，根据主题选择至少两个能够产生不同学习体验的方向；宽泛或 exhaustive 需求选择至少三个。不要为满足数量加入与主题无关的课程、文档或练习。详细规则见 `references/query-strategy.md` 的“互补形态覆盖”章节。
...
- **互补形态覆盖**：未限定形态时，计划至少覆盖两个与主题相关且学习体验不同的方向；宽泛或 exhaustive 需求至少覆盖三个。只在文件材料确有帮助时使用 `filetype:` 查询。
