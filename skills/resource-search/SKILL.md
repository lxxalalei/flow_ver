...
每份计划都加入一个 `generic` 任务；每条 generic 搜索的 `params.engines` 必须包含 `duckduckgo`，可按需额外加入 `bing` 或 `baidu`，用于发现未接入站点、长尾网页和具体文件。
...
- 恰好包含一个 `generic` 任务，且每条 generic 搜索的 `params.engines` 都包含 `duckduckgo`。
