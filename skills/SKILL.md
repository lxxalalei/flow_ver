...
复杂任务基于当前上下文形成以下即时判断；简单、目标明确的请求直接选择对应能力，不必逐项展开全部框架：
...
路线数量由尚未满足的实质学习价值决定：只有用户目标确实包含不同交付价值时才拆分 Coverage；窄任务一条路线即可覆盖时直接推进，不为了覆盖分类表而补充用户没有要求的路线；新增路线无法带来新的实质价值时进入 Stop 判断。
