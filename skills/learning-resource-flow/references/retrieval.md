...
3. 资源实际给谁使用？当前上下文已有的年龄、年级、基础水平等是否与本次对象明确对应并且真的会影响检索？
...
只替换同义词、追加“优质/精品/高赞”等词不能算新的方向。年龄、年级、教材版本、文件格式等只有确实改善当前召回时才进入 query；已知背景也不需要机械复制到每条 query。
...
`target_fit` 使用当前真正相关的背景，而不是机械要求年龄标签。已知年龄/年级且会影响理解门槛、内容尺度或教材范围时可以参与判断；未知时不要自动判 fail，也不要因为缺年龄元数据删除所有候选。

如果候选覆盖不同年龄/阶段但都可能有价值，可以在展示时说明差异，让用户选择；只有年龄/阶段差异会使当前候选明显不可安全/有效使用且无法通过现有证据判断时，才形成 Clarify Gap。

多资源对象时只使用当前对象的背景，不跨对象迁移年龄、年级、偏好或使用场景。
...
- 横向比较任务只覆盖一个来源族；
- 高潜候选是否包含所需内容仍无法判断；
- 已知有多个资源对象，当前对象不明确且背景差异会明显改变内容适配。
