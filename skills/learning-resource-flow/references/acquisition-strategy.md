...
Acquisition Router 是 Job 执行路径中的内部组件。0022 只在已有输出上增加可选的 AssetBundle
投影，不改变公开工具名、`contract_version=1.0.0`、确认令牌、Plan digest、Job 生命周期
状态或 Archive 输入，也不把内部路由名暴露为新的 MCP 工具。Job `status` 不增加 `partial`；
`completion=complete|partial` 只表示结果完整度。
...
取消结果为 `cancelled` 并 quarantine，不得把两者伪装成 partial。

## 三种内部策略

| 策略 | 适用资源 | 默认性 | 输出方向 |
|---|---|---|---|
| `direct_file` | 已验证的 PDF、EPUB、视频、音频、图书或其他直接文件 | 对文件型资源优先 | 原始文件及服务端 Asset 元数据 |
| `web_materialize` | 普通文章、古诗文页、图文博客、可静态读取的网页 | 网页默认 | 安全 HTML、可读 Markdown、元数据、受控同源资产和 ZIP |
| `web_capture` | 必须依赖浏览器执行、且获得显式允许的动态页面快照 | 从不自动选择 | 仅在受控 snapshot profile 可用时产生结果 |

Skill 只根据用户目标、资源表示、已核验事实和获取风险形成语义判断，并解释服务端返回
的计划、警告和结果。服务端 Acquisition Router 才能读取权威资源来源并选择实际执行器，
再次校验权限、来源状态、内容形态、大小预算和取消信号。

### 策略选择规则

1. 资源已经被 `resource_inspect` 或服务端来源事实证明是可直接读取的文件时，优先使用
   `direct_file`。不能因为文件扩展名或搜索摘要就把未知响应当作文件。
2. 普通网页默认使用 `web_materialize`。物化过程不执行页面 JavaScript，不把原始页面
...
Bundle。单资源的旧 Provider 仍可生成 singleton primary Bundle。
...
0022 已完成 Artifact/Asset/AssetBundle 领域边界、七种公开角色、旧 Provider/SmartEdu 映射、
取消 quarantine、重启不自动重放、migration 5、服务层跨资源持久化和 Archive/Library 关系
接入。根智能体的本地固定夹具全量回归为 348/348；真实杀进程、真实 OpenClaw 对话与平台
网络验收进入 0023，本参考文档不代表生产就绪。

现有本地固定夹具、定向契约 Schema 检查和历史 0021 回归不等于真实平台网络、合法会话、
OpenClaw doctor/probe 或多租户生产隔离已经验收。Skill 只能依据 MCP 返回的权威状态恢复和
解释结果，不得把设计规则当作运行时事实。
