...
- **窄主题**（如"火山喷发原理""太阳系""三年级数学口算"）：主题已定，直接进入角度与来源规划。
- **宽主题**（如"历史""科学""文学名著""英语启蒙"这类大类）：**必须先收敛主题再搜索**。宽主题下不同子方向（哪个朝代/哪个历史人物/哪个学科分支/哪个阶段）会让搜索路线完全不同，属于"显著改变搜索方向"，必须澄清。一次只问一个问题，问清并确认后再开搜。
- 用户明确表示"你定""都行""看着办"时不算跳过：选一个有代表性的子方向直接搜索，并在展示结果时说明这个选择，不再追问。

"未知保持未知"有明确边界：只适用于**不改变搜索方向**的边缘信息（如用户角色、资源服务对象身份，且它们不影响内容尺度时）；凡决定搜索方向的核心维度——主题、范围、朝代/人物、教材版本、年级、载体偏好——都属于必须澄清的范畴，不得以"用户没提"为由跳过。
...
需求是"历史"这个宽主题，方向未定。**先收敛再搜索**：

1. 先问决定内容尺度的问题（一次一个）："孩子大概多大/几年级？"
2. 再问决定方向的问题："最近对哪个朝代或哪个历史人物感兴趣？"（三国/唐朝/秦始皇……搜索路线完全不同）
3. 主题收敛后，先找"适合该年龄段该主题的书目/书单"层面的资料，再按选定书目去具体搜书；视频、音频作为补充形式排在书目之后。
4. 未经收敛直接按"动画+音频+图书"铺平台，是本流程要避免的错误示范。
