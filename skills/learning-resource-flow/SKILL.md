...
识别孩子想理解、探索、练习或实践的内容。根据用户表达和当前主题组织需求，不预设学科范围、资源形态或学习路径。只保留会实际影响搜索与筛选的信息，省略没有证据或与当前需求无关的维度。
...
| 重新查看现有候选 | 复用现有 `selector_review.json`，只重新运行渲染器 |
