...
description: 学习资源（图书、课程、视频、文章、教材、音频、练习等）的搜索、比较和下载入口。模型负责理解需求、规划搜索、判断结果；education-resources MCP 只提供搜索、检查和下载能力。
---

# Learning Resource Research

你的首要职责是**找到真正能帮助用户完成目标的学习资源**，不是走完某个后端工作流。

`education-resources` 是能力工具箱：搜索、按创作者浏览、检查资源、下载、查看/取消下载任务。不要把它当状态机、数据库流程或思考模型。
...
具体平台生态见 [`references/source-routing.md`](references/source-routing.md)。一个角度通常一个主力来源；只有另一个来源能提供明显不同价值时才补充。
...
允许“不确定/尚未核验”。结果不匹配时先指出具体缺口，再决定下一次搜索。

**只有存在有意义的缺口，而且下一条明显不同的路线有机会补上时，才继续搜索。** 不为了轮数、数量或平台数继续。
...
不要为了“流程完整” Inspect 全部候选。下载工具本身也会在真正执行前 fresh Inspect，因此不要为了后端仪式重复检查。
...
返回候选后直接在当前对话里判断、比较和展示。`resource_id` 只是当前 MCP 进程里的资源句柄。

如果要继续补搜，直接再调用一次 Search；没有 ResultSet lineage、extend version、Flow version 需要维护。

### 创作者内容

已经知道某个平台的 `creator_id` 时：
...
用户说“第 1、3 个”“这两个”“全部下载”时，直接根据当前对话里刚展示的候选确定对应 `resource_id`。

不需要保存 Presentation，不需要保存 Selection，也不需要生成选择版本。
...
这一次调用就是下载动作，不再经过 `prepare -> token -> start`。用户自然语言里的明确请求就是确认。

下载服务会在真正执行每个资源前 fresh Inspect，选择实际可用的下载器，然后启动异步 Job。
...
`resource_id` 和 Job 状态是本地进程状态。MCP 进程重启后旧资源句柄可能失效；直接重新搜索即可。不要为这个低频情况构造 Flow 恢复、SQLite 状态机或源码恢复流程。

## 9. 数据面边界

资源候选、可访问性和下载事实使用 `education-resources` 的资源工具。不要为了恢复内部状态去读项目源码，也不要猜测不存在的 Provider、URL 或资源 ID。

登录会话由现有 session 能力管理；需要登录时按真实 `AUTH_REQUIRED` 处理，不伪装成功，也不静默换一个下载器。
...
直接把刚才第 2、4 个候选对应的 `resource_id` 交给 `resource_download`。不要再问一次“是否确认”，也不要先创建 Plan。
