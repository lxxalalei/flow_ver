...
Python stdio MCP 当前运行，公共 catalog 为 `1.3.0`，**包含 13 个工具**：
...
`resource_search` 和 `resource_browse_creator` 都只产生 ResultSet 候选。只有模型实际展示后由
`resource_presentation_save` 保存的有序集合，才是可供用户选择的 Presentation。ResultSet 中未进入当前 Presentation
的隐藏候选不得被选择。

`resource_inspect` 可以在 Presentation 保存前对当前 Flow 中的单个候选做有界核验；它创建
独立 Resolution，不改变 ResultSet、Presentation 或 Selection 的不可变/绑定语义。
...
每个 SearchTask 可携带 `direction` 语义目的；它不是服务端 ID，也不能替代平台或 Query。
省略 `mode` 等价于 `replace`；`extend` 必须提供当前 Flow 的
`base_result_set_id`。`replace` 不接受 base。

`extend` 按 base 顺序加本轮候选、跨轮身份去重并创建新的不可变 ResultSet。旧 ResultSet
及其资源行保持不变，新快照中的每个资源获得新的 `resource_id`。服务端在写事务中重新校验
Flow、task version、base 所属关系和 current ResultSet；并发使用陈旧 base 的请求返回稳定冲突。

每个成功搜索持久化 `round`、platform/query runs、去重计数 provenance 和服务端事实
coverage。它们用于恢复与解释，不表示模型已经证明所有语义约束满足；目标充分性、Gap 和
StopDecision 仍由 Skill 基于用户目标和服务端事实判断。
...
### 4.1 `resource_browse_creator`

`resource_browse_creator` 用于按创作者主页获取内容列表，不接受关键词 `search_tasks`，也不直接保存 Presentation。
它在当前 Flow 中执行一次受幂等保护的创作者浏览，并通过同一 ResultSet 持久化候选、平台运行和失败信息。

输入边界与 `schemas/tools/resource_browse_creator.schema.json` 一致：

- `contract_version` 必须为 `1.0.0`；`flow_id`、当前 `task_version` 和 `idempotency_key` 必须有效。
- `platform` 是 1--64 字符的规范平台 ID；`creator_id` 是平台语义下的 1--512 字符不透明标识。
- `limit` 可选，整数范围为 1--50，默认 50。
- 输入对象禁止额外字段；不接受 Cookie、Token、本地路径、脚本、命令、任意下载 URL、`search_tasks` 或 `filters`。

成功输出固定处于 `stage = reviewing`，并包含 `flow_id`、`task_version`、`search_run_id`、`result_set_id`、
`result_version`、`status`、`platform_runs`、`candidates`、`failures`、`has_more` 和 `created_at`。该 ResultSet 是不可变的待审查候选集，
不是用户已看到的内容，也不是 Selection；Skill 必须先审查并实际展示，再通过 `resource_presentation_save` 保存有序子集。
服务端会过滤无效 HTTP(S) 来源和空标题，按私有 Resource Identity 做保守逻辑去重，并为
保留候选生成随机服务端 `resource_id`。

当前默认 Adapter 中实现 `search_creator` 的平台为 `bilibili`、`douyin`、`zhihu` 和 `weibo`。其他教育、资料或普通搜索平台
即使能够执行 `resource_search`，也不因此支持创作者浏览；不支持的平台返回空候选并在 ResultSet 的失败信息中记录
`FEATURE_NOT_SUPPORTED`。未注册的平台记录 `UNKNOWN_PLATFORM`。认证不足、平台不可用或其他适配器失败必须如实记录，
不得伪造候选、绕过登录或访问控制。

### 4.2 私有 Retrieval 与 Platform Registry

公共 ResultSet candidate Schema 保持不变。服务内部可以使用
`CandidateResourceInternal`、`ResourceIdentity`、`Representation` 和
`ResolvedResource` 完成归一化与身份判断，但这些内部对象和 Identity 不能作为工具输入或
权限标识。

Identity 优先级为平台 native ID、ISBN、DOI、平台感知 canonical URL、标题/作者/版本弱
指纹。默认 URL 处理只移除 fragment；平台查询参数只能按
`platforms/platform-registry.json` 的 Identity profile 清理。去重保持首次出现顺序，后续
候选只补充缺失事实，强身份冲突不自动合并；`limit` 在去重后应用。公共 `resource_id` 只在
最终公共投影时由服务端随机生成，Adapter 提供的 ID 不得直接沿用。

Platform Registry 当前包含 `generic` 与 15 个内置平台。每个内置 Adapter 和 generic 搜索
后端都必须暴露与 Registry 一致的冻结 descriptor；Registry 与登录态 Session Registry 分离。
0019 精确启用 `generic`、`bilibili`、`nlc`、`annas-archive`、`ximalaya`、`zhihu`、`smartedu`
七个平台 inspect；其余平台由精确 Router 返回 `FEATURE_NOT_SUPPORTED`，不静默回退 generic。

### 4.3 `resource_inspect` 与 Resolution
...
输入对象禁止额外字段；不接受 URL、路径、批量资源 ID、检查深度、Cookie、Token 或其他凭据。
服务端必须确认 `resource_id` 属于该 `flow_id`，再从 Flow 资源重新读取来源并执行有界检查。

成功输出固定包含 `flow_id`、`resource_id`、服务端生成的 `resolution_id`、
`resolution_status`（`resolved`、`partial`、`unresolved`）、`resolved_resource`、`inspection`
和 `failures`。`resolved_resource` 可以包含 `representations`，每项必须使用服务端生成的
`representation_id`（`repr_...`）和受控元数据；不得返回 locator、文件字节、本地路径或凭据。
`availability.status` 只使用 `available`、`auth_required`、`unavailable`、`unknown`、
`policy_blocked`。

首版使用固定 profile `inspect-v1` 和有界同步检查。Generic 必须采用 GET、逐跳 SSRF/重定向
校验、1 MiB 上限以及 MIME/魔数交叉验证；平台 Inspector 必须保留合法的可用性和失败状态，
不绕过登录、验证码、付费墙、DRM 或访问控制。Inspect 不下载、不启动 Job、不归档。

Resolution 通过 SQLite migration 3 的 `resource_resolutions` 独立持久化，ResultSet 快照
保持不可变。幂等 scope 为 `resource_inspect:{flow_id}`；缓存键为
`resource_id + source_fingerprint + profile_version`，其中 `resolved`/`partial` 可跨新键
缓存，`unresolved` 仅保存审计/恢复并由新幂等键重试。`resource_flow_status` 的
`current_resolutions` 只返回当前 ResultSet 范围内的安全 Resolution 摘要。
...
### 8.1 AssetBundle 与部分结果

`AssetBundle` 是一个 Job 针对一个 Resource 产生的有序多资产关系，不等于 ZIP、目录或任意
本地路径。它由服务端生成 `bundle_id`，并通过 BundleItem 关联已验证 Asset 或保留失败事实。
一个可用 Bundle 必须有且只有一个 `primary`；失败 BundleItem 的 `asset_id` 为空，不创建零字节
假 Asset。单 Asset 和 0021 的 Web ZIP 可以作为没有伴随项的 singleton primary。

公共 AssetRole 固定为以下七种，不能只依据文件名或扩展名推断：

```text
primary | subtitle | cover | metadata | attachment | transcript | companion
```

`completion`/`bundle_completion` 只表达 Bundle 结果完整度：`complete` 表示预期项全部成功，
`partial` 表示已有可用 primary 但一个或多个预期项失败。它不加入 Job `status`；Job 生命周期
仍使用 `queued`、`running`、`cancelling`、`succeeded`、`failed`、`cancelled`。

因此，`resource_job_status.success`、`resource_flow_status.current_job`、Asset summary、
Archive success、Library Asset 和 item failure 可以按各自 Schema 追加可选的
`bundle_id`、`role`、`order`、`bundle_completion`、`completion`、`bundle_ids` 或 `item_key`。
这些是服务端只读投影；旧单 Asset 输出省略它们时，既有 required 和含义保持不变。
...
- Resolution 内部的来源 locator、文件字节或未受控大对象；
- 可用于绕过服务端校验的内部存储键。
...
| `resolution_id` | `resolve_<opaque>` | 一次服务端资源核验的持久化结果 |
| `representation_id` | `repr_<opaque>` | Resolution 中受控表示的服务端 ID |
| `plan_id` | `plan_<opaque>` | 有期限的下载准备结果 |
| `job_id` | `job_<opaque>` | 异步下载任务 |
| `asset_id` | `asset_<opaque>` | 服务端受控且已校验资产 |
| `bundle_id` | `bundle_<opaque>` | 一个 Job × Resource 的服务端有序 AssetBundle 关系 |
...
返回 `relative_path`。Asset 属于 Bundle 时还可以返回可选的 `bundle_id`、`role`、`order`、
`bundle_completion`。兼容字段 `library_path` 如仍返回，必须与 `relative_path` 一样是资料库根下的相对路径。
