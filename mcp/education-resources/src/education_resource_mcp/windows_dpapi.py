...
_DPAPI_ENTROPY_PREFIX = b"education-resource-mcp\x00windows-dpapi-v1\x00"
...
class WindowsDpapiProtector:
    ...
    def _last_error(operation: str) -> WindowsDpapiError:
        return WindowsDpapiError(
            f"Windows DPAPI {operation}失败（错误码 {ctypes.get_last_error()}）"
        )
    ...
    def protect(self, plaintext: bytes, *, purpose: str) -> bytes:
        ...
        if not self._crypt32.CryptProtectData(
            ctypes.byref(source), "Education Resource MCP", ctypes.byref(entropy),
            None, None, CRYPTPROTECT_UI_FORBIDDEN, ctypes.byref(output),
        ):
