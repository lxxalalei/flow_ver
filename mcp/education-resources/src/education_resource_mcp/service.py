...
from .capability import CapabilityAuthorityError, CapabilityCoordinator
...
class ResourceService:
    def __init__(
        ...
        *,
        ...
        capability_registry_snapshot: RegistrySnapshot | None = None,
        ...
        if rendering_downloader is not None:
            raise ValueError(
                "rendering_downloader is not an executable capability; "
                "register an exact provider in acquisition_router and declare it "
                "in capability_registry_snapshot"
            )

        provider_import_errors: dict[str, dict[str, str]] = {}
        if acquisition_router is None:
            ...
            try:
                ...
                provider_import_errors["smartedu-resource"] = {
        ...
        self.capability_coordinator = CapabilityCoordinator(
            store=self.store,
            registry_snapshot=capability_registry_snapshot,
            runtime_inventory={
                "provider_registry": self.acquisition_router.provider_registry,
                "registered_inspectors": self.inspection_router.registered_inspectors,
        ...
        *,
        ...
        if unknown_options:
            raise DomainError(
                "INVALID_ARGUMENT",
                "options 包含不支持的字段",
                details={"fields": ",".join(unknown_options)},
            )
        ...
        try:
            capability_items = self.capability_coordinator.prepare_selection(
                flow_id,
                resources,
                resolutions,
                preferred_container=str(container),
        ...
        try:
            ...
            if error in {
                ...
                "capability_binding_resource_mismatch",
                "duplicate_capability_binding",
                "duplicate_capability_binding_digest",
            }:
                raise DomainError(
            ...
            if error in {
                ...
                "invalid_capability_binding_position",
                "invalid_capability_representation",
                "invalid_capability_scope",
                "invalid_capability_strategy",
                "capability_representation_too_large",
            }:
                raise DomainError("VALIDATION_ERROR", "下载计划能力绑定无效") from exc
            raise DomainError("INTERNAL_ERROR", "下载计划持久化失败") from exc
        except RuntimeError as exc:
            mapping = {
            ...
            code, message = mapping.get(
                str(exc), ("FLOW_STATE_CONFLICT", "下载准备状态冲突")
            )
            raise DomainError(code, message) from exc
    ...
    def download_start(
        ...
        *,
        ...
        for field, value in (
            ...
            if not isinstance(value, str) or BARE_SHA256_PATTERN.fullmatch(value) is None:
        ...
        if authority_digest is not None and (
            ...
            or BARE_SHA256_PATTERN.fullmatch(authority_digest) is None
        ):
            raise DomainError("INVALID_ARGUMENT", "authority_digest 必须是 SHA-256 摘要")
        ...
        # An accepted request owns an immutable execution snapshot.  Replay it
        # before loading mutable Plan/Resolution/readiness facts so later
        # deployment drift cannot invalidate the same idempotency identity.
        try:
            replayed_job = self.store.lookup_download_start_replay(
            ...
            if replay_error == "execution_binding_missing":
                raise DomainError(
                    "CAPABILITY_BINDING_CONFLICT",
                    "既有任务缺少不可伪造的执行能力绑定",
                ) from exc
            if replay_error == "idempotency record points to a missing job":
                raise DomainError(
                    "INTERNAL_ERROR",
                    "下载任务幂等记录损坏",
                ) from exc
        ...
        if replayed_job is not None:
            replayed_plan = self.store.get_plan(plan_id)
            replayed_authority_digest = (
                str(replayed_plan.get("authority_digest") or "")
                if replayed_plan is not None
                else ""
            )
            if BARE_SHA256_PATTERN.fullmatch(replayed_authority_digest) is None:
                raise DomainError(
                    "CAPABILITY_BINDING_CONFLICT",
                    "既有任务缺少有效的计划能力权威摘要",
                )
            return {
                ...
                "selection_digest": selection_digest,
                "plan_digest": plan_digest,
                "authority_digest": replayed_authority_digest,
                "job_id": replayed_job["job_id"],
                "status": "queued",
                "queued_at": replayed_job["created_at"],
            }

        plan = self.store.get_plan(plan_id)
        if plan is None or plan["flow_id"] != flow_id:
            raise DomainError("PLAN_NOT_FOUND", "下载计划不存在")
        effective_authority_digest = str(plan.get("authority_digest") or "")
        if BARE_SHA256_PATTERN.fullmatch(effective_authority_digest) is None:
            raise DomainError(
                "CAPABILITY_BINDING_CONFLICT",
                "下载计划缺少有效的能力权威摘要",
            )
        if authority_digest is not None and authority_digest != effective_authority_digest:
            raise DomainError(
        ...
        capability_items = plan.get("capability_items")
        resource_ids = list(plan.get("resource_ids") or [])
        if (
            not isinstance(capability_items, list)
            or not capability_items
            or len(capability_items) != len(resource_ids)
            or [str(item.get("resource_id") or "") for item in capability_items]
            != resource_ids
        ):
            raise DomainError(
                "CAPABILITY_BINDING_CONFLICT",
        ...
        if [str(resource.get("resource_id") or "") for resource in resources] != resource_ids:
            raise DomainError(
                "RESOLUTION_STALE",
                "下载计划中的资源已经变化，请重新检查并准备下载",
            )

        execution_bindings: list[dict[str, Any]] = []
        for plan_item, resource in zip(capability_items, resources, strict=True):
            try:
                fingerprint = source_fingerprint(resource)
            except (TypeError, ValueError) as exc:
                raise DomainError(
            ...
            try:
                revalidated = self.capability_coordinator.assert_revalidate_plan_item(
                    plan_item,
                    resource,
                    resolution,
                    flow_id=flow_id,
            ...
            execution_binding = revalidated.get("execution_binding")
        ...
        try:
            ...
            raise DomainError("VALIDATION_ERROR", "下载执行绑定无效") from exc
        except RuntimeError as exc:
            mapping = {
                ...
                "readiness_not_ready": (
                    ...
                    "下载能力当前未就绪，请重新检查并准备下载",
                ),
                "readiness_expired": (
                    "READINESS_EXPIRED",
                    "下载能力就绪证据已过期，请重新检查并准备下载",
                ),
                "readiness_drift": (
                    "READINESS_DRIFT",
                    "下载能力部署状态已变化，请重新检查并准备下载",
                ),
                "eligibility_required": (
                    "ELIGIBILITY_REQUIRED",
                    "当前资源缺少可执行的资格或授权，请重新检查",
                ),
                "eligibility_expired": (
                    "ELIGIBILITY_EXPIRED",
                    "资源资格或授权证据已过期，请重新检查并准备下载",
                ),
                "eligibility_drift": (
                    "ELIGIBILITY_DRIFT",
                    "资源资格、授权或策略事实已变化，请重新检查",
                ),
                "resolution_stale": (
                    "RESOLUTION_STALE",
                    "资源检查结果已经变化，请重新检查并准备下载",
                ),
                "representation_drift": (
                    "REPRESENTATION_DRIFT",
                    "已确认的资源表示已经变化，请重新检查并准备下载",
                ),
                "idempotency record points to a missing job": (
                    "INTERNAL_ERROR",
                    "下载任务幂等记录损坏",
                ),
            ...
            code, message = mapping.get(
                str(exc), ("FLOW_STATE_CONFLICT", "下载状态冲突")
            )
            raise DomainError(code, message) from exc
        if not reused:
    ...
    def job_status(self, flow_id: str, job_id: str) -> dict[str, Any]:
        ...
        try:
            execution_by_resource = {
                ...
                for item in self.store.get_job_execution_items(job_id)
            }
        except RuntimeError:
            # Legacy jobs remain readable, but missing execution authority is
            # never reconstructed from Plan options or provider metadata.
            execution_by_resource = {}
        outcomes = [
            self._public_acquisition_outcome(
                outcome,
                execution_by_resource.get(str(outcome["resource_id"])),
            )
        ...
        authority_digest = str(plan.get("authority_digest") or "")
        if BARE_SHA256_PATTERN.fullmatch(authority_digest):
    ...
    def _run_download_job(self, job_id: str, cancel_event: threading.Event) -> None:
        """Execute only the immutable authority reserved for ``job_id``.

        A job has no executable fallback path: its Router request is assembled
        from ``job_execution_items`` and the nested Resource input returned by
        that Store query.  Plan options, resource type, downloader seams, and
        platform inference are deliberately absent from this runner.
        """
        ...
        try:
            ...
            execution_items = self.store.get_job_execution_items(job_id)
            ...
            for execution in execution_items:
                ...
                request = AcquisitionRequest(
                    ...
                    planned_scope=execution["capability_scope"],
                    representation_id=execution["representation_id"],
                    binding_digest=execution["execution_binding_digest"],
                    ...
                    readiness_snapshot_id=execution["readiness_snapshot_id"],
                    readiness_digest=execution["readiness_digest"],
                ...
                expected_actual = (
                    str(execution["capability_scope"]),
                    str(execution["strategy"]),
                    str(execution["provider_id"]),
                    str(execution["provider_version"]),
                )
                observed_actual = (
    ...
    def _public_acquisition_outcome(
        ...
        """Project persisted planned/execution/actual facts without locators.

        The Store owns every field used here.  Provider response metadata,
        local paths, request locators and credentials are intentionally not
        copied into ``resource_job_status``.
        """
        ...
        planned = {
            ...
            "plan_binding_digest": str(outcome["plan_binding_digest"]),
        }
        execution_digest = outcome.get("execution_binding_digest")
        execution_projection: dict[str, Any] | None = None
        if (
            isinstance(execution_digest, str)
            and re.fullmatch(r"[a-f0-9]{64}", execution_digest) is not None
        ):
            execution_projection = {"binding_digest": execution_digest}
        if (
            execution_projection is not None
            and execution is not None
            and str(execution.get("execution_binding_digest") or "")
            == execution_digest
        ):
            execution_projection.update(
                {
                    "scope": str(execution["capability_scope"]),
                    ...
                    "capability": {
                        ...
                        "descriptor_version": str(execution["descriptor_version"]),
                        "descriptor_digest": str(execution["descriptor_digest"]),
                        "registry_version": str(execution["registry_version"]),
                        "registry_digest": str(execution["registry_digest"]),
                        "readiness_snapshot_id": str(
                            execution["readiness_snapshot_id"]
                        ),
                        "readiness_digest": str(execution["readiness_digest"]),
                    },
                    "eligibility": {
                        "eligibility_id": str(execution["eligibility_id"]),
                        "decision_digest": str(execution["eligibility_digest"]),
                    },
                    "source_fingerprint": str(execution["source_fingerprint"]),
        ...
        result: dict[str, Any] = {
            ...
            "outcome_digest": str(outcome["outcome_digest"]),
        }
        if execution_projection is not None:
            result["execution"] = execution_projection
        ...
        if isinstance(asset_ids, list) and asset_ids:
            result["asset_ids"] = [str(asset_id) for asset_id in asset_ids[:50]]
        if outcome.get("failure_code") is not None:
            failure_code = str(outcome["failure_code"])
            result["failure"] = {
                "code": (
                    failure_code
                    if failure_code in PUBLIC_JOB_FAILURE_CODES
